...
def expand_tree_instances(tree: VSSNode) -> VSSNode:
    ...
    def create_instantiated_branch(branch_name, parent, nodes_to_expand):
        ...
        instantiated_branch = VSSNode(
            branch_name,
            {"type": "branch", "description": parent.description,
                "comment": parent.comment, "$file_name$": "Generated"},
...
def find_branch_or_struct(elem, name_list, index, autocreate=True):
    ...
    if name_list[index] not in children:
        if autocreate and elem['type'] != "struct":
            ...
            if "$name$" not in elem:
                elem['$name$'] = ""
            newelem = {'type': elem['type'], 'children': {
            }, '$line$': '0', '$file_name$': '<generated>', '$name$': f"{elem['$name$']}.{name_list[index]}"}
